package com.pdfconverter.jpg2pdf.pdf.converter.ui.mergepdf.done;
...
import com.pdfconverter.jpg2pdf.pdf.converter.ui.mergepdf.MergePdfNavigator;
import com.pdfconverter.jpg2pdf.pdf.converter.ui.mergepdf.MergePdfViewModel;
...
import com.pdfconverter.jpg2pdf.pdf.converter.utils.merge.MergePdfListener;
import com.pdfconverter.jpg2pdf.pdf.converter.utils.merge.MergePdfManager;

import java.io.File;

import cn.pedant.SweetAlert.SweetAlertDialog;

public class MergePdfDoneActivity extends BaseBindingActivity<ActivityCreatePdfDoneBinding, MergePdfViewModel> implements MergePdfListener, MergePdfNavigator {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mOptions = new Gson().fromJson(json, MergePDFOptions.class);
        if (mOptions == null || mOptions.getPathList() == null) {
    ...
    private void setForLayoutView() {
        if (mCreatingStatus == STATE_NOT_STARTED) {
            ...
            if (mCreatingStatus == STATE_DONE) {
                ...
                mActivityCreatePdfDoneBinding.createLayout.createStatusText.setText(getString(R.string.add_watermark_creating_success_pdf));
        ...
        RenameFileDialog renameFileDialog = new RenameFileDialog(this, displayName, new RenameFileDialog.RenameFileListener() {
            ...
            public void onSubmitName(String name) {
                ...
                if (result == -2 || result == 0) {
                    ...
                    SnackBarUtils.getSnackbar(MergePdfDoneActivity.this, getString(R.string.duplicate_video_name) + ": " + name).show();
                } else {
                    SnackBarUtils.getSnackbar(MergePdfDoneActivity.this, getString(R.string.rename_file_success)).show();
                    ...
                    mMergePdfViewModel.updateSavedData(tempOldDir, mOutputPath);
                }
            }
    ...
    public void onCreateSuccess(String finalOutput) {
        runOnUiThread(() -> {
            ...
            mMergePdfViewModel.saveRecent(mOutputPath, getString(R.string.merge_pdf));
    ...
    private void startCreatePdf() {
        mTaskManager = new MergePdfManager(this, this, mOptions);
