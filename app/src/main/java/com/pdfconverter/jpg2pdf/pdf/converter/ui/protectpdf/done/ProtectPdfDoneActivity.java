package com.pdfconverter.jpg2pdf.pdf.converter.ui.protectpdf.done;
...
import com.pdfconverter.jpg2pdf.pdf.converter.ui.protectpdf.ProtectPdfNavigator;
import com.pdfconverter.jpg2pdf.pdf.converter.ui.protectpdf.ProtectPdfViewModel;
...
public class ProtectPdfDoneActivity extends BaseBindingActivity<ActivityCreatePdfDoneBinding, ProtectPdfViewModel> implements ProtectPdfNavigator {
    ...
    private void setForLayoutView() {
        if (mCreatingStatus == STATE_NOT_STARTED) {
            ...
            if (mCreatingStatus == STATE_DONE) {
                ...
                mActivityCreatePdfDoneBinding.createLayout.createStatusText.setText(getString(R.string.add_watermark_set_password_success_pdf));
        ...
        RenameFileDialog renameFileDialog = new RenameFileDialog(this, displayName, new RenameFileDialog.RenameFileListener() {
            ...
            public void onSubmitName(String name) {
                ...
                if (result == -2 || result == 0) {
                    ...
                    SnackBarUtils.getSnackbar(ProtectPdfDoneActivity.this, getString(R.string.duplicate_video_name) + ": " + name).show();
                } else {
                    SnackBarUtils.getSnackbar(ProtectPdfDoneActivity.this, getString(R.string.rename_file_success)).show();
                    ...
                    mProtectPdfViewModel.updateSavedData(tempOldDir, mOutputPath);
                }
            }
    ...
    private void startCreatePdf() {
        ...
        AsyncTask.execute(() -> {
            ...
            try {
                mOutputPath = PdfUtils.setPasswordPdfFile(this, mFilePath, mPassword);

                runOnUiThread(() -> {
                    if (mOutputPath != null) {
                        mActivityCreatePdfDoneBinding.createLayout.txtProgress.setText("100 %");

                        mProtectPdfViewModel.saveRecent(mOutputPath, getString(R.string.protect_pdf));

                        mCreatingStatus = STATE_DONE;
                    } else {
                        mCreatingStatus = STATE_FAIL;
                        ToastUtils.showMessageShort(this, getString(R.string.protect_pdf_add_password_error));
                    }
