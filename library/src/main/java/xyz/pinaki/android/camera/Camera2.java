...
//            updateFlash(); TODO: add later
    ...
    void captureStillPicture() {
        try {
            ...
            switch (flashType) {
                case FLASH_AUTO:
                    picCaptureRequestBuilder.set(CaptureRequest.CONTROL_AE_MODE, CaptureRequest.CONTROL_AE_MODE_ON_AUTO_FLASH);
                    break;
                case FLASH_ON:
                    picCaptureRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_SINGLE);
                    break;
                case FLASH_OFF:
                    picCaptureRequestBuilder.set(CaptureRequest.FLASH_MODE, CaptureRequest.FLASH_MODE_OFF);
                    break;
            }
...
//        deviceOrientation = (deviceOrientation + 45) / 90 * 90;
        ...
        public void onImageAvailable(ImageReader reader) {
            try (Image image = reader.acquireNextImage()) {
                ...
                if (planes.length > 0) {
                    ...
                    int cameraRotation = getJpegOrientation(cameraCharacteristics);

                    final Bitmap rotatedBitmap = Bitmap.createBitmap(bitmap, 0, 0, bitmap.getWidth(), bitmap
                            .getHeight(), getImageTransformMatrix(cameraRotation), false);

                    uiHandler.post(new Runnable() {
                        @Override
                        public void run() {
                            cameraStatusCallback.onBitmapProcessed(rotatedBitmap, cameraRotation);
                        }
                    });
                }
            }
    ...
    private Matrix getImageTransformMatrix(int cameraRotation) {
